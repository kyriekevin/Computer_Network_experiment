...
#define LIMIT 524288 // 流量限制
#define STATISTIC_TIME 30000 // 统计数据长度时间间隔间隔30s
...
pcap_if_t* all_devs; // 设备列表
FILE* fp = NULL; // 输出文件

ULONGLONG last_sec; // 警报间隔计时

MAC_ADDR self_mac_addr; // 本设备MAC地址
...
pcap_if_t* selectDev(int DevsCount) {
	...
	if (choice <= 0 || choice > DevsCount) {
		printf("设备号应在(1-%d)间，实际输入为%d，不符合规范！\n", DevsCount, choice);
...
int main() {
	...
	pcap_t* handle = getHandle(current_dev); // 获取句柄
